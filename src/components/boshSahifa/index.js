...
import { faPhone, faBars, faSun, faMoon, faSquareXmark } from "@fortawesome/free-solid-svg-icons";
import {
  faFacebook,
  faInstagram,
  faSquareXing,
    ...
    <div class={`mainBox mb-5 ${theme=='dark' ? '' : 'rang'}`}>
      <div className={`themeBox`}>
        <button id="button" onClick={Exchange}>
          {change == true ? <FontAwesomeIcon icon={faSquareXmark} /> : <FontAwesomeIcon icon={faBars} />}
        </button>
      ...
      <h1 className={`text-${theme=='dark'?'light':'dark'} fw-bold text-center`}> Iqboljon Shuhratov </h1>
      <p className={`text-${theme=='dark'?'light':'dark'} fs-1 text-center mb-4`}> Frontend </p>
